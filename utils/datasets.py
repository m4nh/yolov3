...
class LoadImagesAndLabelsDarknetStyle:  # for training
    def __init__(self, path, batch_size=1, img_size=608, multi_scale=False, augment=False):
        ...
        self.img_files = [path.split(' ')[0] for path in self.rows]
        self.labels = [path.split(' ')[1:] for path in self.rows]
    ...
    def __next__(self):
        ...
        for index, files_index in enumerate(range(ia, ib)):
            img_path = self.img_files[self.shuffled_vector[files_index]]
            labels_list = self.labels[self.shuffled_vector[files_index]]
            ...
            if len(labels_list)>0:
                labels0 = np.array([list(map(float, l.split(','))) for l in labels_list])
                # Normalized xywh to pixel xyxy format
                labels0 = np.roll(labels0, 1, axis=1).astype(np.float32)
